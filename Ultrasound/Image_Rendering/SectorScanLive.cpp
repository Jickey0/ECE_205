...
class SectorScanLive {
    private:
        ...
        int scanStartXPos, scanStartYPos, gradientXPos, gradientYPos, scanRadius = 160, 30, 5, 24, 300;
    ...
    public:
        ...
        int displayScanLine(){
            ...
            for (int i = 0; i < ADC_SAMPLES; i++) {
                int x, y;
                polarToCartesian(scan2D[currentScanIndex].roll, i * radiusScale, x, y, scanStartXPos, scanStartYPos); // roll corresponds to angle and i corresponds to radius
                uint16_t color = convertToColor(scan2D[currentScanIndex].values[i]);
